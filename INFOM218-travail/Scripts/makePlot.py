...
def count_query_types(file_path):
    ...
    for query in data.get("Queries", []):
        ...
        for query_type, pattern in patterns.items():
            ...
            if match:
                table_name = match.group(1) if query_type != 'drop' else match.group(2) #get first subset of alphabetical characters in most cases
...
if __name__ == "__main__":
    ...
    plt.xlabel('Tables')
    plt.ylabel('Nombre')
    plt.title('Nombre de requêtes par table')
